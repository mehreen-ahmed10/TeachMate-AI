    ...
    if st.button("✨ Generate Worksheet", type="primary", use_container_width=True, key="generate_ws"):
        if not w_topic.strip():
            ...
            with st.spinner("Creating worksheet..."):
                result = run_safely(generate_worksheet, w_grade, w_subject, w_topic, w_difficulty, int(w_count), w_types)
            if result:
                display_output(render(result), w_subject)
...
with tabs[2]:
    ...
    if st.button("✨ Generate Assessment", type="primary", use_container_width=True, key="generate_assessment"):
        if not a_topic.strip():
            st.warning("Please enter a topic.")
        else:
            with st.spinner("Creating assessment..."):
                result = run_safely(generate_assessment, a_grade, a_subject, a_topic, int(a_marks), a_difficulty, a_types)
            if result:
                display_output(render(result), a_subject)
